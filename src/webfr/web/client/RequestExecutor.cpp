  ...
  if(!m_retryPolicy) {
    ...
    while(true) {
      ...
      while(tick < tick0 + waitMicro) {
        ...
        tick = webfr::base::Environment::getMicroTickCount();
      }
...
webfr::async::CoroutineStarterForResult<const std::shared_ptr<RequestExecutor::Response>&>
RequestExecutor::executeAsync(
  ...
  class ExecutorCoroutine : public webfr::async::CoroutineWithResult<ExecutorCoroutine, const std::shared_ptr<RequestExecutor::Response>&> {
  private:
