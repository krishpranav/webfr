...
RequestHeadersReader::Result RequestHeadersReader::readHeaders(data::stream::InputStreamBufferedProxy* stream,
  ...
  while(!iteration.done) {
    ...
    if(!action.isNone()) {
      WEBFR_LOGE("[webfr::web::protocol::http::incoming::RequestHeadersReader::readHeaders]", "Error. Async action is unexpected.");
      throw std::runtime_error("[webfr::web::protocol::http::incoming::RequestHeadersReader::readHeaders]: Error. Async action is unexpected.");
    }
  ...
  if(error.ioStatus > 0) {
    webfr::parser::Caret caret (m_bufferStream->getData(), m_bufferStream->getCurrentPosition());
    http::Status status;
...
webfr::async::CoroutineStarterForResult<const RequestHeadersReader::Result&>
RequestHeadersReader::readHeadersAsync(const std::shared_ptr<data::stream::InputStreamBufferedProxy>& stream)
{
  
  class ReaderCoroutine : public webfr::async::CoroutineWithResult<ReaderCoroutine, const Result&> {
  private:
  ...
  public:
    ...
    Action act() override {
      ...
      return error<Error>("[webfr::web::protocol::http::incoming::RequestHeadersReader::readHeadersAsync()]: Error. Error reading connection stream.");

    }
    
    Action parseHeaders() {

      webfr::parser::Caret caret (m_this->m_bufferStream->getData(), m_this->m_bufferStream->getCurrentPosition());
      http::Status status;
      ...
      if(status.code == 0) {
        ...
        if(status.code == 0) {
          ...
          return error<Error>("[webfr::web::protocol::http::incoming::RequestHeadersReader::readHeadersAsync()]: Error. Error occurred while parsing headers.");
        }
      } else {
        return error<Error>("[webfr::web::protocol::http::incoming::RequestHeadersReader::readHeadersAsync()]: Error. Can't parse starting line.");
      }
