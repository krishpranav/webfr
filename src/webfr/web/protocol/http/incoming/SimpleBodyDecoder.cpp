                                       ...
                                       const data::share::StringKeyLabelCI& contentEncoding ) const
{
  ...
  if(contentEncoding && !m_contentDecoders) {
    throw std::runtime_error("[webfr::web::protocol::http::incoming::SimpleBodyDecoder::getStreamProcessor()]: "
  ...
  if(transferEncoding) {
    ...
    if(contentLengthStr) {
      ...
      if(connectionStr && connectionStr == "close") {
        ...
        throw std::runtime_error("[webfr::web::protocol::http::incoming::SimpleBodyDecoder::decode()]: Error. Invalid Request.");
      }
...
async::CoroutineStarter SimpleBodyDecoder::decodeAsync(const Headers& headers,
  ...
  throw std::runtime_error("[webfr::web::protocol::http::incoming::SimpleBodyDecoder::decodeAsync()]: Error. Invalid Request.");
