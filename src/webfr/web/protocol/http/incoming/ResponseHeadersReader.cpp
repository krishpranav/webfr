...
ResponseHeadersReader::Result ResponseHeadersReader::readHeaders(const std::shared_ptr<webfr::data::stream::IOStream>& connection,
  ...
  webfr::data::stream::BufferOutputStream buffer;
  ...
  while(!iteration.done) {
    ...
    if(!action.isNone()) {
      throw std::runtime_error("[webfr::web::protocol::http::incoming::ResponseHeadersReader::readHeaders]: Error. Async action is unexpected.");
    }
...
webfr::async::CoroutineStarterForResult<const ResponseHeadersReader::Result&>
ResponseHeadersReader::readHeadersAsync(const std::shared_ptr<webfr::data::stream::IOStream>& connection)
{
    ...
    Action parseHeaders() {
      ...
      if(status.code == 0) {
        ...
        if(status.code == 0) {
          ...
          return error<Error>("[webfr::web::protocol::http::incoming::ResponseHeadersReader::readHeadersAsync()]: Error. Error occurred while parsing headers.");
        }
      } else {
        return error<Error>("[webfr::web::protocol::http::incoming::ResponseHeadersReader::readHeadersAsync()]: Error. Can't parse starting line.");
      }
