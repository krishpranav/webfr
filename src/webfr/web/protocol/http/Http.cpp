...
const char* const Header::Value::SERVER = "webfr/" WEBFR_VERSION;
const char* const Header::Value::USER_AGENT = "webfr/" WEBFR_VERSION;
...
webfr::String Range::toString() const {
...
Range Range::parse(webfr::parser::Caret& caret) {
  ...
  auto start = webfr::utils::conversion::strToInt64((const char*) startLabel.getData());
  auto end = webfr::utils::conversion::strToInt64((const char*) endLabel.getData());
  return Range(unitsLabel.toString(true), start, end);
  
}

Range Range::parse(const webfr::String& str) {
  webfr::parser::Caret caret(str);
  return parse(caret);
}

webfr::String ContentRange::toString() const {
  webfr::data::stream::ChunkedBuffer stream;
  stream.writeSimple(units->getData(), units->getSize());
  stream.writeSimple(" ", 1);
  stream.writeAsString(start);
...
ContentRange ContentRange::parse(webfr::parser::Caret& caret) {
  ...
  v_int64 start = webfr::utils::conversion::strToInt64((const char*) startLabel.getData());
  v_int64 end = webfr::utils::conversion::strToInt64((const char*) endLabel.getData());
  v_int64 size = 0;
  bool isSizeKnown = false;
  if(sizeLabel.getData()[0] != '*') {
    isSizeKnown = true;
    size = webfr::utils::conversion::strToInt64((const char*) sizeLabel.getData());
  }
...
ContentRange ContentRange::parse(const webfr::String& str) {
  webfr::parser::Caret caret(str);
  return parse(caret);
}


webfr::String HeaderValueData::getTitleParamValue(const data::share::StringKeyLabelCI& key) const {
...
webfr::data::share::StringKeyLabelCI_FAST Parser::parseHeaderNameLabel(const std::shared_ptr<webfr::base::StrBuffer>& headersText,
                                                                         webfr::parser::Caret& caret) {
  p_char8 data = caret.getData();
  for(v_buff_size i = caret.getPosition(); i < caret.getDataSize(); i++) {
    v_char8 a = data[i];
    if(a == ':' || a == ' '){
      webfr::data::share::StringKeyLabelCI_FAST label(headersText, &data[caret.getPosition()], i - caret.getPosition());
      caret.setPosition(i);
      return label;
      
    }
  }
  return webfr::data::share::StringKeyLabelCI_FAST(nullptr, nullptr, 0);
}
...
void Parser::parseRequestStartingLine(RequestStartingLine& line,
  ...
  if(caret.findChar(' ')){
    line.method = webfr::data::share::StringKeyLabel(headersText, methodLabel.getData(), methodLabel.getSize());
    caret.inc();
  } else {
  ...
  if(caret.findChar(' ')){
    line.path = webfr::data::share::StringKeyLabel(headersText, pathLabel.getData(), pathLabel.getSize());
    caret.inc();
  } else {
    error = Status::CODE_400;
    return;
  ...
  if(caret.findRN()){
    line.protocol = webfr::data::share::StringKeyLabel(headersText, protocolLabel.getData(), protocolLabel.getSize());
    caret.skipRN();
  } else {
    error = Status::CODE_400;
    return;
...
void Parser::parseResponseStartingLine(ResponseStartingLine& line,
  ...
  if(caret.findRN()){
    line.description = webfr::data::share::StringKeyLabel(headersText, descriptionLabel.getData(), descriptionLabel.getSize());
    caret.skipRN();
  } else {
    error = Status::CODE_400;
    return;
                            ...
                            Status& error)
{
  ...
  if(name.getData() != nullptr) {
    ...
    caret.findRN();
    headers.put_LockFree(name, webfr::data::share::StringKeyLabel(headersText, &caret.getData()[valuePos0], caret.getPosition() - valuePos0));
    caret.skipRN();
  } else {
...
void Parser::parseHeaderValueData(HeaderValueData& data, const webfr::data::share::StringKeyLabel& headerValue, v_char8 separator) {

  webfr::parser::Caret caret(headerValue.getData(), headerValue.getSize());
