    ...
    switch(connectionState) {
      ...
      case ConnectionState::DELEGATED: {
        ...
        if(handler) {
          ...
          WEBFR_LOGW("[webfr::web::server::HttpProcessor::processNextRequest()]", "Warning. ConnectionUpgradeHandler not set!");
          connectionState = ConnectionState::CLOSING;
        }
        break;
      }
...
HttpProcessor::Coroutine::Coroutine(const std::shared_ptr<Components>& components,
  ...
  , m_headersOutBuffer(std::make_shared<webfr::data::stream::BufferOutputStream>(components->config->headersOutBufferInitial))
...
webfr::async::Action HttpProcessor::Coroutine::onHeadersParsed(const RequestHeadersReader::Result& headersReadResult) {
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::onResponseFormed() {
  ...
  webfr::web::protocol::http::utils::CommunicationUtils::considerConnectionState(m_currentRequest, m_currentResponse, m_connectionState);
  ...
  switch(m_connectionState) {
    ...
    case ConnectionState::DELEGATED: {
      ...
      if(handler) {
        ...
        WEBFR_LOGW("[webfr::web::server::HttpProcessor::Coroutine::onResponseFormed()]", "Warning. ConnectionUpgradeHandler not set!");
        m_connectionState = ConnectionState::CLOSING;
      }
      break;
    }
...
HttpProcessor::Coroutine::Action HttpProcessor::Coroutine::handleError(Error* error) {
  ...
  if(error) {
    ...
    if(error->is<webfr::AsyncIOError>()) {
      auto aioe = static_cast<webfr::AsyncIOError*>(error);
      if(aioe->getCode() == webfr::IOError::BROKEN_PIPE) {
        return aioe; 
      }
    }
